...
class Association:
    ...
    def associate(self, track_list, meas_list, KF):
        ...
        self.association_matrix = np.matrix(
            [np.inf*np.ones(len(track_list), len(meas_list))])
        self.unassigned_tracks = [range(len(track_list))]
        self.unassigned_meas = [range(len(meas_list))]
        for track_num, track_obj in enumerate(track_list):
            for meas_num, measuremenmt in enumerate(meas_list):
                MHD_value = self.MHD(track_obj, measuremenmt, KF)
                if self.gating(MHD_value, measuremenmt.sensor):
                    self.association_matrix[track_num, meas_num] = MHD_value
    ...
    def gating(self, MHD, sensor):
        ...
        # if MHD < chi2.ppf(params.gating_threshold, df=2):
        #     return True
    ...
    def MHD(self, track, meas, KF):
        ...
        MHD_value = np.transpose(res)*np.linalg.inv(S)*res
        return MHD_value
